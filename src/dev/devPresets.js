...
export async function createDevPreset(name, { gameTime, currentLoads = seedLoads, currentDrivers = seedDrivers, currentRuntimePositions = {}, currentCarriers = [], selectedLoadId = 'DOC001' } = {}) {
  ...
  const roster = reconcileActiveCarrierDrivers(currentDrivers, currentCarriers); const driver = roster.find((x) => x.id === 'marcus'); if (name !== 'available' && !driver) throw new Error('Activate Metroline before using driver load presets.'); const pickup = mapLocations.find((x) => x.id === load.pickupLocationId); const delivery = mapLocations.find((x) => x.id === load.deliveryLocationId); const now = nowOf(gameTime); const base = { selectedMarket: 'new-york', gameTime: gameTime || { gameDayIndex: 0, totalMinutesOfDay: 360 }, loads: [load], drivers: roster, carriers: currentCarriers, runtimePositions: { ...currentRuntimePositions }, runtimeProgress: null, stage: 'game' }; const withDriver = (position) => ({ ...base, drivers: roster.map((x) => x.id === 'marcus' ? { ...x, status: 'unavailable', assignedLoadId: load.id } : x), runtimePositions: position ? { ...currentRuntimePositions, marcus: position } : { ...currentRuntimePositions } })
