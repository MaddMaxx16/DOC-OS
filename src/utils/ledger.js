import { getFreightRouteName } from './freightIdentity.js'
export function getReceivables(loads = [], carriers = [], workflows = {}) {
  return loads.filter((load) => load.tripStatus === 'completed' && load.pod?.approved).map((load) => {
    ...
    return { loadId: load.id, loadNumber: getFreightRouteName(load), routeName: getFreightRouteName(load), carrierId: carrier.id, carrierName: carrier.name, carrierGross: load.rate, agreementType: agreement.type, agreementPercentage: percentage, dispatchRevenue: load.rate * percentage / 100, receivableStatus: 'OUTSTANDING', financialStatus: workflow.financialStatus || 'READY_TO_INVOICE', ...workflow, paymentAvailableGameMinute: workflow.paymentAvailableGameMinute ?? (Number.isFinite(workflow.invoiceSentGameMinute) ? workflow.invoiceSentGameMinute + 1440 : null) }
  }).filter(Boolean)
}
