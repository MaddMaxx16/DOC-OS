...
export function getMarcusPanelModel({ assignedLoad, gameTime, runtimeProgress = 0, pickup, delivery }) {
  ...
  const eta = operationalState === 'EN_ROUTE_PICKUP' && Number.isFinite(assignedLoad.departureGameMinute) ? assignedLoad.departureGameMinute + (assignedLoad.plannedDeadheadDriveTimeMinutes || 0) : operationalState === 'EN_ROUTE_DELIVERY' && Number.isFinite(assignedLoad.deliveryDepartureGameMinute) ? assignedLoad.deliveryDepartureGameMinute + (assignedLoad.plannedLoadedDriveTimeMinutes || 0) : null
  const labels = { ASSIGNED: 'Assigned', TRIP_PLANNED: 'Trip Planned', EN_ROUTE_PICKUP: 'En Route to Pickup', WAITING_PICKUP: 'Waiting at Pickup', CHECKED_IN_PICKUP: 'Checked In at Pickup', LOADING: 'Loading', LOADED: 'Loaded', READY_FOR_DISPATCH: 'Ready for Dispatch', EN_ROUTE_DELIVERY: 'En Route to Delivery', WAITING_DELIVERY: 'Waiting at Delivery', CHECKED_IN: 'Checked In', UNLOADING: 'Unloading', COMPLETED: 'Completed' }
  const actions = { ASSIGNED: ['PLAN_TRIP', 'PLAN TRIP'], TRIP_PLANNED: ['SEND_TO_PICKUP', 'SEND TO PICKUP'], EN_ROUTE_PICKUP: ['EN_ROUTE', 'EN ROUTE'], WAITING_PICKUP: ['CHECK_IN', 'CHECK IN'], CHECKED_IN_PICKUP: ['CHECKED_IN', 'CHECKED IN'], LOADING: ['LOADING', 'LOADING'], LOADED: ['PLAN_DELIVERY_TRIP', 'PLAN DELIVERY TRIP'], READY_FOR_DISPATCH: ['DISPATCH', 'DISPATCH'], EN_ROUTE_DELIVERY: ['EN_ROUTE', 'EN ROUTE'], WAITING_DELIVERY: ['CHECK_IN', 'CHECK IN'], CHECKED_IN: ['CHECKED_IN', 'CHECKED IN'], UNLOADING: ['UNLOADING', 'UNLOADING'], COMPLETED: [null, null] }
  ...
  const locationLabel = ['WAITING_PICKUP', 'CHECKED_IN_PICKUP', 'LOADING', 'LOADED'].includes(operationalState) ? pickup?.name : ['WAITING_DELIVERY', 'CHECKED_IN', 'UNLOADING'].includes(operationalState) ? delivery?.name : null
  return { operationalState, statusLabel: labels[operationalState], loadId: assignedLoad.id, nextStopLabel: nextStop, locationLabel, eta: eta === null ? null : `${formatCompactDate(Math.floor(eta / 1440))} • ${formatTime(eta % 1440)}`, progress: runtimeProgress, actionType, actionLabel, actionDisabled: disabled, attentionRequired: ['WAITING_PICKUP', 'LOADED', 'READY_FOR_DISPATCH', 'WAITING_DELIVERY'].includes(operationalState), remainingMinutes: operationalState === 'WAITING_PICKUP' && Number.isFinite(assignedLoad.pickupArrivalGameMinute) ? Math.max(0, 5 - (now - assignedLoad.pickupArrivalGameMinute)) : operationalState === 'UNLOADING' && Number.isFinite(assignedLoad.deliveryUnloadStartGameMinute) ? Math.max(0, 8 - (now - assignedLoad.deliveryUnloadStartGameMinute)) : null, plannedDeadheadMiles: assignedLoad.plannedDeadheadMiles, plannedDeadheadDriveTimeMinutes: assignedLoad.plannedDeadheadDriveTimeMinutes, plannedLoadedMiles: assignedLoad.plannedLoadedMiles, plannedLoadedDriveTimeMinutes: assignedLoad.plannedLoadedDriveTimeMinutes, pickupWindow: pickup ? formatAppointment(assignedLoad.pickupDayIndex, assignedLoad.pickupWindowStartMinutes, assignedLoad.pickupWindowEndMinutes) : null }
}
