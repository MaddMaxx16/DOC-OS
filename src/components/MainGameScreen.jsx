...
function MainGameScreen({ selectedMarket, gameTime, loads, setLoads, drivers, setDrivers, carriers, onActivateCarrier, carrierApplicationsById, onApplyCarrier, onAcceptAgreement, emailMessages, setEmailMessages, tutorialEnabled = false, plannedRoute, setPlannedRoute, setGameClockPaused, runtimePositions, runtimeProgress, setRuntimeProgress, simulationSpeed, setSimulationSpeed, onOpenMarkets, onResetGame, seenLedgerReceivableIds, seenLedgerPaymentReadyIds, onOpenLedger, ledgerWorkflowByLoadId, setLedgerWorkflowByLoadId, setGameTime }) {
  ...
  const tutorialDriverAction = (() => {
    if (!tutorialEnabled || isPhoneOpen || driverFitEvaluation || planningMode || deliveryPlanning || doc001?.assignedDriverId !== 'marcus') return null
    if (doc001.tripStatus === 'assigned') return doc001.planningStatus === 'route-ready' ? 'SEND_TO_PICKUP' : 'PLAN_TRIP'
    if (doc001.tripStatus === 'waiting-at-pickup') {
      const now = gameTime.gameDayIndex * 1440 + gameTime.totalMinutesOfDay
      const remainingWaitMinutes = Number.isFinite(doc001.pickupArrivalGameMinute)
        ? Math.max(0, PICKUP_WAIT_MINUTES - (now - doc001.pickupArrivalGameMinute))
        : 0
      return remainingWaitMinutes <= 0 ? 'CHECK_IN' : null
    }
    if (doc001.tripStatus === 'at-pickup') return 'CHECK_IN'
    if (doc001.tripStatus === 'loaded') return doc001.deliveryPlanningStatus === 'route-ready' ? 'DISPATCH' : 'PLAN_DELIVERY_TRIP'
    return null
  })()
