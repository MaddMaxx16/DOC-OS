...
function MainGameScreen({ selectedMarket, gameTime, loads, setLoads, drivers, setDrivers, plannedRoute, setPlannedRoute, setGameClockPaused, runtimePositions, runtimeProgress, onOpenMarkets }) {
  ...
  return (
    <div className="main-game-screen">
      ...
      <div className="map-area">
        <GameMap drivers={drivers} plannedRoute={plannedRoute} deadheadRoute={driverFitEvaluation?.deadheadRoute || (planningMode?.route?.routeShape) || assignedLoad?.plannedDeadheadRouteGeometry} runtimePositions={runtimePositions} runtimeProgress={runtimeProgress} runtimeRoute={assignedLoad?.plannedDeadheadRouteGeometry} assignedLoad={assignedLoad} gameTime={gameTime} onPlanTrip={(loadId, driverId) => { const load = loads.find((item) => item.id === loadId); if (load?.planningStatus === 'route-ready' && load.plannedDeadheadRouteGeometry) { setLoads((current) => current.map((item) => item.id === loadId ? { ...item, tripStatus: 'en-route-pickup', departureGameMinute: gameTime.gameDayIndex * 1440 + gameTime.totalMinutesOfDay } : item)) } else { startPlanning(loadId, driverId) } }} />
