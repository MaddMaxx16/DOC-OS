...
function MainGameScreen({ selectedMarket, gameTime, loads, setLoads, drivers, setDrivers, plannedRoute, setPlannedRoute, setGameClockPaused, runtimePositions, runtimeProgress, simulationSpeed, setSimulationSpeed, onOpenMarkets }) {
  ...
  const startDeliveryPlanning = async (loadId) => {
    ...
    try { setDeliveryPlanning({ loadId, route: await calculateRoute(pickup, delivery) }) } catch (error) { console.error(error); setDeliveryPlanning({ loadId, route: 'unavailable' }) }
  }
  ...
  return (
    <div className="main-game-screen">
      ...
      <div className="map-area">
        <div className="simulation-speed">{[1, 2, 4].map((speed) => <button key={speed} type="button" className={speed === simulationSpeed ? 'active' : ''} onClick={() => setSimulationSpeed(speed)}>{speed}×</button>)}</div>
        <GameMap drivers={drivers} plannedRoute={deliveryPlanning ? null : plannedRoute} deadheadRoute={deliveryPlanning?.route?.routeShape || driverFitEvaluation?.deadheadRoute || planningMode?.route?.routeShape || assignedLoad?.plannedDeadheadRouteGeometry} runtimePositions={runtimePositions} runtimeProgress={runtimeProgress} runtimeRoute={assignedLoad?.plannedDeadheadRouteGeometry} assignedLoad={assignedLoad} suppressAttention={Boolean(deliveryPlanning)} gameTime={gameTime} onPlanTrip={(loadId, driverId) => { const load = loads.find((item) => item.id === loadId); if (load?.tripStatus === 'loaded' && !load.plannedLoadedRouteGeometry) startDeliveryPlanning(loadId); else if (load?.planningStatus === 'route-ready' && load.plannedDeadheadRouteGeometry) setLoads((current) => current.map((item) => item.id === loadId ? { ...item, tripStatus: 'en-route-pickup', departureGameMinute: gameTime.gameDayIndex * 1440 + gameTime.totalMinutesOfDay } : item)); else startPlanning(loadId, driverId) }} />
