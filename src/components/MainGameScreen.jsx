  ...
  return (
    <div className="main-game-screen">
      ...
      <div className="map-area">
        {import.meta.env.DEV && <><button type="button" className="dev-button" onClick={() => setDevOpen((open) => !open)}>DEV</button>{devOpen && <div className="dev-menu"><div className="dev-menu-header"><strong>DEV TOOLS</strong><button type="button" onClick={() => setDevOpen(false)} aria-label="Close developer tools">×</button></div><div className="dev-presets">{['no-client', 'metroline-active', 'driver-fit', 'accepted', 'ready-for-pickup-dispatch', 'en-route-pickup', 'waiting-at-pickup', 'loading', 'loaded', 'ready-for-delivery-dispatch', 'en-route-delivery', 'waiting-at-delivery', 'unloading', 'pod-ready', 'pod-verified', 'load-complete'].map((preset) => <button type="button" key={preset} onClick={() => { onApplyDevPreset(preset); setDevOpen(false) }}>{preset.replaceAll('-', ' ').toUpperCase()}</button>)}</div><button type="button" className="dev-reset" onClick={() => { onResetGame(); setDevOpen(false) }}>RESET GAME</button></div>}</>}
